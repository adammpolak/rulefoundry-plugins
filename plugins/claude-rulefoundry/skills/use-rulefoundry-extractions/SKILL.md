...
This public plugin may list workspaces and accessible Extractions, search and
read Extraction metadata, list or read completed artifacts, and create one new
request with self or an external email participant after explicit confirmation. It must never
begin, conduct, resume, continue, complete, modify, resend, or delete an
Extraction or artifact. An Extraction session still happens in RuleFoundry.
